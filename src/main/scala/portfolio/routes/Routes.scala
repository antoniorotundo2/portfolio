...
object Routes:
  ...
  val routes: Routes[PortfolioService, Nothing] = Routes(

    // ── Static files ──────────────────────────────────────────────────────────
    Method.GET / "static" / "css" / string("file") ->
      handler { (file: String, _: Request) =>
        ZIO.attemptBlocking {
          val path = s"/static/css/$file"
    ...
    Method.GET / "" ->
      handler { (_: Request) =>
        ...
        } yield html(HomeView.render(profile, projects, posts))
      },
    ...
    Method.GET / "projects" ->
      handler { (_: Request) =>
        for {
          svc      <- ZIO.service[PortfolioService]
          projects <- svc.getProjects
        } yield html(ProjectsView.render(projects))
      },
    ...
    Method.GET / "blog" ->
      handler { (_: Request) =>
        for {
          svc   <- ZIO.service[PortfolioService]
          posts <- svc.getBlogPosts
        } yield html(BlogView.render(posts))
      },
    ...
    Method.GET / "blog" / string("slug") ->
      handler { (slug: String, _: Request) =>
        ...
        } yield post.fold(notFound)(p => html(BlogPostView.render(p)))
      },
